...
with subtest("Check that custom activation messages were logged correctly"):
    assert "must appear during activation (via Nix)" in logs
    assert "must appear during activation (custom)" in logs
